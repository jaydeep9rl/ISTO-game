...
public class MainGamePanel extends SurfaceView implements
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (explosion == null || explosion.getState() == Explosion.STATE_DEAD) {
			explosion = new Explosion(EXPLOSION_SIZE, w / 2, h / 2);
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (explosion == null
					|| explosion.getState() == Explosion.STATE_DEAD) {
...
//		canvas.drawLines(
//				new float[] { 0, 0, canvas.getWidth() - 1, 0,
//						canvas.getWidth() - 1, 0, canvas.getWidth() - 1,
//						canvas.getHeight() - 1, canvas.getWidth() - 1,
//						canvas.getHeight() - 1, 0, canvas.getHeight() - 1, 0,
//						canvas.getHeight() - 1, 0, 0 }, paint);
	}
